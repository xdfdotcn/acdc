...
const FieldMappingList: React.FC = (props) => {
	const {fieldMappingModel, setFieldMappings} = useModel('FieldMappingModel');

	const showHandleColumn: boolean = fieldMappingModel.from == 'detail'
	...
	const initDataSource = (items: API.FieldMappingListItem[]) => {
		...
		setFieldMappings(loadedTableItems);
	}
	...
	const fieldMappingColumns: ProColumns<API.FieldMappingListItem>[] = [
		...
		},
		{
			...
			valueType: "select",
			hideInTable: fieldMappingModel.sinkDataSystemType == 'KAFKA',
			width: "35%",
	...
	*/
	const fetchFieldMenu = async (itemList: API.FieldMappingListItem[]) => {
		// 源表id
		const srcDataSetId = fieldMappingModel.srcDataSetId
	...
	return (
		<EditableProTable<API.FieldMappingListItem, API.FieldMappingQuery>
			...
			params={{
				connectorId: fieldMappingModel.connectorId,
				from: fieldMappingModel.from,
				srcDataSystemType:fieldMappingModel.srcDataSystemType,
				srcDataSetId: fieldMappingModel.srcDataSetId,
				sinkDataSetId: fieldMappingModel.sinkDataSetId,
				sinkDataSystemType: fieldMappingModel.sinkDataSystemType,
			}}
			request={fetchFieldMapping}
			rowClassName={(record) => {
				if (fieldMappingModel.sinkDataSystemType == 'KAFKA') {
					return
				}
