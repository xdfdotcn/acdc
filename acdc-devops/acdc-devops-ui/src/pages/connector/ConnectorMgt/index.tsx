...
import { stopConnector, startConnector } from '@/services/a-cdc/api';
      ...
      render: (text, record, _, action) => [
        <>
          {record.desiredState == 'RUNNING' ? (
            <a
              onClick={() => {
                startOrStopConnector(record.id!, false);
              }}
            >
              <PlaySquareOutlined />
              停止
            ...
            <a
              onClick={() => {
                startOrStopConnector(record.id!, true);
              }}
            >
              <PoweroffOutlined />
              启动
    ...
    <div>
      <ProTable<API.ConnectorListItem, API.ConnectorQuery>
        params={{
          ...
          refreshVersion: loadV,
        }}
        rowKey={(record) => record.id + '_' + loadV}
        // 请求数据API
