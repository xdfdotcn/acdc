...
public class AbstractDataSystemSinkConnectorServiceTest {
    ...
    public void testGenerateConnectorName() {
        ConnectionDTO connection = new ConnectionDTO().setId(1L).setSourceDataCollectionId(2L).setSinkDataCollectionId(3L);

        List<DataSystemResourceDTO> sourceDataCollectionPath = Lists.newArrayList(
                new DataSystemResourceDTO().setId(11L).setDataSystemType(DataSystemType.MYSQL).setResourceType(DataSystemResourceType.MYSQL_CLUSTER),
                new DataSystemResourceDTO().setName("db1"),
                new DataSystemResourceDTO().setName("tb1")
        );

        List<DataSystemResourceDTO> sinkDataCollectionPath = Lists.newArrayList(
                new DataSystemResourceDTO().setId(12L).setDataSystemType(DataSystemType.HIVE).setResourceType(DataSystemResourceType.HIVE),
                new DataSystemResourceDTO().setName("db2"),
                new DataSystemResourceDTO().setName("tb2")
        );

        when(connectionService.getById(connection.getId())).thenReturn(connection);

        when(dataSystemResourceService.getPathById(connection.getSourceDataCollectionId())).thenReturn(sourceDataCollectionPath);

        when(dataSystemResourceService.getPathById(connection.getSinkDataCollectionId())).thenReturn(sinkDataCollectionPath);
    ...
    public void testGetPathFormatById() {
        ...
        List<DataSystemResourceDTO> dataCollectionPath = Lists.newArrayList(
                mysqlClusterDataSystemResource,
                mysqlDatabaseDataSystemResource,
                mysqlTableDataSystemResource
        );

        when(dataSystemResourceService.getPathById(any())).thenReturn(dataCollectionPath);
    ...
    public void testGetPathFormatByIdWhenNoParentResource() {
        ...
        when(dataSystemResourceService.getPathById(dataSystemResource.getId())).thenReturn(dataCollectionPath);
