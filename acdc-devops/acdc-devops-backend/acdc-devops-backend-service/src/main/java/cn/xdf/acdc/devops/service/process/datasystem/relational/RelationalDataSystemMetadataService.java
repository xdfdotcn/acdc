...
public abstract class RelationalDataSystemMetadataService implements DataSystemMetadataService {
    ...
    protected HostAndPort getHostAndPort(final DataSystemResourceDetailDTO instance) {
        ...
        int port = Integer.parseInt(instance.getDataSystemResourceConfigurations().get(Instance.PORT.getName()).getValue());
    ...
    private void refreshDatabase(
        ...
        List<DataSystemResourceDetailDTO> actualDatabases = generateResourceDetails(actualDatabaseNames, getDatabaseDataSystemResourceType(), clusterResourceDetail);
    ...
    private void refreshTable(
            ...
            final UsernameAndPassword usernameAndPassword,
            final Set<HostAndPort> hostAndPorts) {
        List<DataSystemResourceDetailDTO> databaseResources = dataSystemResourceService.getDetailChildren(clusterResourceDetail.getId(), getDatabaseDataSystemResourceType());
        databaseResources.forEach(each -> refreshTableOfDatabase(each, usernameAndPassword, hostAndPorts));
    }
    
    private void refreshTableOfDatabase(
            final DataSystemResourceDetailDTO databaseResourceDetail,
            final UsernameAndPassword usernameAndPassword,
            final Set<HostAndPort> hostAndPorts) {
        List<String> actualTableNames = mysqlHelperService.showTables(hostAndPorts, usernameAndPassword, databaseResourceDetail.getName());
        List<DataSystemResourceDetailDTO> actualTables = generateResourceDetails(actualTableNames, getTableDataSystemResourceType(), databaseResourceDetail);
        dataSystemResourceService.mergeAllChildrenByName(actualTables, getTableDataSystemResourceType(), databaseResourceDetail.getId());
    }
    ...
    public DataCollectionDefinition getDataCollectionDefinition(final Long dataCollectionResourceId) {
        ...
        List<DataFieldDefinition> dataFieldDefinitions = mysqlHelperService
                ...
                .map(it -> new DataFieldDefinition(
                        it.getName(),
                        it.getType(),
                        RelationalDataSystemDataCollectionDefinitionUtil.getConnectSchema(it.getType()),
                        it.isOptional(),
                        it.getDefaultValue(),
                        RelationalDataSystemDataCollectionDefinitionUtil.getFieldExtendProperties(it.getType()),
