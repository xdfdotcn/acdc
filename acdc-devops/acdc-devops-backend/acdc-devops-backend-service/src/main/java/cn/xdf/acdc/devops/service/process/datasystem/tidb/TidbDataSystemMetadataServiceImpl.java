...
public class TidbDataSystemMetadataServiceImpl extends RelationalDataSystemMetadataService {
    ...
    public TidbDataSystemMetadataServiceImpl() {
        createDataSystemResourceDefinition();
    }

    private void createDataSystemResourceDefinition() {
        DataSystemResourceDefinition tidbTable = new DataSystemResourceDefinition(Collections.EMPTY_MAP, DataSystemResourceType.TIDB_TABLE);

        Map<DataSystemResourceType, DataSystemResourceDefinition> databaseChildren = new HashMap<>();
        databaseChildren.put(tidbTable.getType(), tidbTable);
        DataSystemResourceDefinition tidbDatabase = new DataSystemResourceDefinition(Collections.EMPTY_MAP, DataSystemResourceType.TIDB_DATABASE, databaseChildren);

        DataSystemResourceDefinition tidbServer = new DataSystemResourceDefinition(Collections.EMPTY_MAP, DataSystemResourceType.TIDB_SERVER);

        Map<DataSystemResourceType, DataSystemResourceDefinition> clusterChildren = new HashMap<>();
        clusterChildren.put(tidbServer.getType(), tidbServer);
        clusterChildren.put(tidbDatabase.getType(), tidbDatabase);
        DataSystemResourceDefinition tidbCluster = new DataSystemResourceDefinition(Collections.EMPTY_MAP, DataSystemResourceType.TIDB_CLUSTER, clusterChildren);

        dataSystemResourceDefinition = tidbCluster;
    }
    ...
    private void checkTidbInstance(final DataSystemResourceDetailDTO tidbInstanceDetail) {
        DataSystemResourceDetailDTO clusterDetail = dataSystemResourceService.getDetailById(tidbInstanceDetail.getParentResourceId());
    ...
    protected HostAndPort getHostAndPort(final DataSystemResourceDetailDTO instance) {
        ...
        String host = instance.getDataSystemResourceConfigurations().get(TidbDataSystemResourceConfigurationDefinition.Instance.HOST.getName()).getValue();
        int port = Integer.parseInt(instance.getDataSystemResourceConfigurations().get(TidbDataSystemResourceConfigurationDefinition.Instance.PORT.getName()).getValue());
