...
public class ColumnDefinition {
    ...
    /**
     * Get the column's specified column size. For numeric data, this is the maximum precision.  For character data,
     * this is the length in characters. For datetime datatypes, this is the length in characters of the String
    ...
    /**
     * Gets the column's number of digits to right of the decimal point. 0 is returned for data types where the scale is
     * not applicable.
     *
     * @return scale
     */
     ...
     * {@link java.sql.ResultSet#getObject(int)} is called to retrieve a value from the column.
     * {@link java.sql.ResultSet#getObject(int)} may return a subclass of the class returned by this method.
     *
     * @return the fully-qualified name of the class in the Java programming language that would be used by the method
     *     <code>ResultSet.getObject</code> to retrieve the value in the specified column. This is the class name used
     *     for custom mapping.
     */
    ...
    /**
     * Obtain a {@link ColumnDefinition} that has all the same characteristics as this column except that belongs to the
     *              specified table.
     *
     * @param tableId the new table ID; may not be null
     * @return this definition if the specified table ID matches this definition's {@link #tableId()}, or a new
     *              definition that is a copy of this definition except with an {@link #id() ID} that uses the specified table; never
     *              null
     */
    ...
    /**
     * Obtain a {@link ColumnDefinition} that has all the same characteristics as this column except that it does or does not belong to the table's primary key.
     *
     * @param isPrimaryKey true if the resulting column definition should be part of the table's primary key, or false otherwise
     * @return a definition that is the same as this definition except it is or is not part of the tables primary key,
     *              or may be this object if {@link #isPrimaryKey()} already matches the supplied parameter; never null
     */
