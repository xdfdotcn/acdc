...
/**
 * An abstract provider of {@link DatabaseDialect} instances, and used to {@link #score(JdbcUrlInfo) score} the dialect
 * for a given JDBC URL.
 *
 * <p>The {@link DatabaseDialects} class uses Java's Service Provider API to discover and register
 * all {@link DatabaseDialectProvider} classes that are on the classpath. All of these registered providers are then
 * consulted any time someone attempts to {@link DatabaseDialects#findBestFor(String, AbstractConfig) find the best
 * dialect} for a given URL and JDBC source or sink connector configuration. The dialect providers compute a score for
 * the URL, and the dialect provider with the highest score is then used to create a new {@link DatabaseDialect}
 * instance.
 *
 * <p>To implement a new dialect, extend this class and place the name of the implementation class
 * in a file that will appear in the JAR at {@code META-INF/services/io.confluent.connect.jdbc.dialect.DatabaseDialectProvider}.
 *
 * <p>For example, all of the dialects providers provided by the Confluent JDBC connector are listed
 * in the {@code META-INF/services/io.confluent.connect.jdbc.dialect.DatabaseDialectProvider}. Other JARs can be added
 * to the plugin (or classpath) with additional database dialects.
 */
public abstract class DatabaseDialectProvider {
    ...
    /**
     * The "average" score that should be returned by the {@link #score(JdbcUrlInfo)} method to signal that the
     * DatabaseDialect instance can be used with a connection.
     */
    ...
    /**
     * Return the score describing how well this {@link DatabaseDialect} instance works with the given JDBC URL and
     * {@link Connection}. The DatabaseDialect that scores the highest for a given connection will be used.
     *
     * @param jdbcInfo the information about the JDBC connection URL; may not be null
     * @return the score; may be {@link #NO_MATCH_SCORE} if the dialect does not apply, or greater than or equal to
     * {@link #NO_MATCH_SCORE} if the DatabaseDialect can be used with this connection.
     */
    ...
    public interface JdbcUrlInfo {
        ...
        /**
         * Get the subname in the JDBC URL, which is everything after the ':' character following the subprotocol.
         *
         * @return the subname
         */
        String subname();
    ...
    /**
     * An abstract base class for a provider that will return a {@link DatabaseDialectProvider#EXCELLENT_MATCHING_SCORE}
     * if any of the supplied JDBC subprotocols match the subprotocol from the JDBC connection URL, or {@link
