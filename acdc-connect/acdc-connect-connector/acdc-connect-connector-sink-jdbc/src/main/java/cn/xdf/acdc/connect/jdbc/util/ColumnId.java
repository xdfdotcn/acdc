...
public class ColumnId implements ExpressionBuilder.Expressable {
    ...
    /**
     * Gets the column's suggested title for use in printouts and displays. The suggested title is usually specified by
     * the SQL <code>AS</code> clause.  If a SQL <code>AS</code> is not specified, the value will be the same as the
