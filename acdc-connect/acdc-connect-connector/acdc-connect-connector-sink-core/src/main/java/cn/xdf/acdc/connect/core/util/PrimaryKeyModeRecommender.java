...
public class PrimaryKeyModeRecommender implements ConfigDef.Recommender {
    ...
    private static final List<Object> ALL_VALUES =
            Arrays.stream(PrimaryKeyMode.values())
                    .map(mode -> mode.name().toLowerCase())
                    .collect(Collectors.toList());

    private static final List<Object> RECORD_KEY_ONLY = Collections.singletonList(PrimaryKeyMode.RECORD_KEY.name().toLowerCase());

    @Override
    public List<Object> validValues(final String name, final Map<String, Object> parsedConfig) {
        final boolean deleteEnabled = (Boolean) parsedConfig.getOrDefault(SinkConfig.DELETE_ENABLED, false);
