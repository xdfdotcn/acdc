...
public class FieldAdditionProcessTest {
    ...
    public void addStringFieldAndPlaceholderField() {
        ...
        Assert.assertFalse(fieldAndValues.get("acdc_update_time").getValue().equals("${datetime}"));
    }
    ...
    public void shouldCoverValueWhenFieldNameExisted() {
        ...
        Assert.assertEquals(fieldAndValues.size(), 4);
        Assert.assertEquals(fieldAndValues.get("string_field_1").getValue(), "new_value_1");
    }
    ...
    public void testShouldReturnZonedTimestampWhenAddStringFieldAndPlaceholderField() {
        ...
        Assert.assertEquals("+08:00", zonedDateTime.getZone().getId());
        Assert.assertEquals(ZonedTimestamp.LOGICAL_NAME, fieldAndValues.get("acdc_update_time").getSchema().name());
    }
}
