...
public class TransactionTableRegion implements TableRegion {
    ...
    @Override
    public int write(SinkData<byte[]> sinkData) {
        if (null == sinkData || null == sinkData.getData()) {
            return 0;
        }
        
        int c;
        if (ctl.compareAndSet(false, true)) {
            c = write0(sinkData.getData());
            offsetTracker.markProcessed(getUniqueKey(), sinkData.getSinkRecord());
        } else {
            for (; ; ) {
                if (ctl.compareAndSet(false, true)) {
                    c = write0(sinkData.getData());
        ...
        if (state.compareAndSet(State.ACTIVE, State.FLUSHING)) {
            for (; ; ) {
                if (ctl.compareAndSet(false, true)) {
                    ...
                    outBuffer = null;
                    ctl.set(false);
                    offsetTracker.flush(getUniqueKey());
                    break;
                }
            }
